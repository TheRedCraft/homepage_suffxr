...
const lazyClass = 'numbers';
const lazytargets = document.querySelectorAll(`.${lazyClass}`);
...
function triggerObserve(target) {
	if (target.id == 'uses') {
		setTimeout(() => {
			old = createNumberArray(0);
			const diff = 116;
			const value = parseInt(old.join('')) + diff;
			animateNumber(value, document.querySelector('#uses'));
		}, 1000);
	}
	if (target.id == 'servers') {
		setTimeout(() => {
			old = createNumberArray(0);
			const diff = 23;
			const value = parseInt(old.join('')) + diff;
			animateNumber(value, document.querySelector('#servers'));
		}, 2000);
	}
}
